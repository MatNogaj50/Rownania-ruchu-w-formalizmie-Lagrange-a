...
if len(sys.argv) > 1:
    try:
        NUMBER_OF_PARTICLES = int(sys.argv[1]) # Number of particles read from command line
...
RADIUS              = 1.0       # Initial radius for particle placement
dt                  = 0.05     # Timestep for the simulation

# Generate EOM from lagrangian.py
try:
    subprocess.run([sys.executable, "lagrangian.py"], check=True)
except subprocess.CalledProcessError:
    print("Lagrangian generation failed")
    sys.exit(1)
